...
It processes the input tree bottom-up,
\amitodo{This is ot true?} and uses registers to store parts of the output tree. The semantics of the transducer model involves two phases: (a) mapping the input tree to an expression that uses register updates; (b) evaluating the expression. We will then show that each of the two phases represents a derivable functions.
