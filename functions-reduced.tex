...
$$\begin{array}{cc}
        \ranked{
        \xymatrix@C=.7cm{
 \tmonad \reduce k \overbrace{(\rSigma \otimes \cdots \otimes \rSigma)}^{\black{\text{$k$ times}}} \ar[r]& \termset +   \reduce k \overbrace{(\tmonad\rSigma \otimes \cdots \otimes \tmonad\rSigma)}^{\black{\text{$k$ times}}} 
        }}
    \end{array}$$ 
\end{itemize}
\end{minipage}
