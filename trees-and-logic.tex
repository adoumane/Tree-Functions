...
\begin{example}\label{ex:pre-order-main} Define  \emph{pre-order} to be the following ordering of  nodes in a tree:  $x$ is before  $y$ if either $x$ is an ancestor of $y$, or there exist siblings $x'$ and $y'$ such that $x'$ is an ancestor of $x$, $y'$ is an ancestor of $y$, and $x'$ has a smaller child number than $y'$.  Consider  the tree-to-tree which transforms a tree into a list of its nodes in pre-order, as explained in the following picture:
    \mypic{112}
    Because the pre-order can be defined in first-order logic,  the above function is first-order tree-to-tree transduction. Unlike Example~\ref{ex:filter-first}, we need copying, because a node of arity $n$ in the input tree corresponds to $n+2$ nodes in the output tree.
\end{example}
