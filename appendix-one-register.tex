...
initial text=$ $,]
\node[state, initial] (q1) {${\scriptstyle i}$};
\node[state,  right of=q1] (q2) {${\scriptstyle p}$};
\node[state, accepting,right of=q2] (q3) {${\scriptstyle q}$};
\draw 
(q1) edge[above] node{$
{\scriptstyle x, \perp\mapsto \sf{push}(\sigma_n),\dots,\sf{push}(\sigma_1)}$} (q2)
(q1) edge[below] node{$
{\scriptstyle \text{if } x: \sigma_1\rightarrow\dots\rightarrow\sigma_n}$} (q2)
(q2) edge[loop above] node{ $\begin{array}{rr}
{\scriptstyle\lambda y, \_\mapsto \sf{push}(\sigma)} & {\scriptstyle\text{if } y:\sigma}\\
{\scriptstyle\text{@}, \gamma\in S \mapsto \sf{pop}}&
\end{array}$} (q2)
(q2) edge[above] node{$
\scriptstyle{\epsilon, \tau \mapsto \sf{pop}}
$} (q3);
\end{tikzpicture}
\end{center}
\end{itemize}
