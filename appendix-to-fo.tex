...
Let $a$ be un element of $\ranked{\tmonad  \reduce 2 (\Sigma \otimes \Sigma)}$. We will explain how
 $\underline{\unfold(a)}$ can be obtained from $\underline{a}$ via a first-order transduction. For that we will need two copies of $\underline{a}$. 
...
\varphi_2(x)= &\mathsf{port}_{\color{blue}{\Sigma}}(x) \vee \mathsf{port}_{\color{green}{\Sigma}}(x) \wedge  
\end{align*}
\end{enumerate}
