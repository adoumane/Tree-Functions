...
As discussed in the body of the paper, we proceed by induction on the derivation. During this induction, we will need to show that every prime function is a first-order transduction. But prime functions are not tree-to-tree functions. Instead they transform dataypes into datatypes. This is the reason why we need to generalize
\begin{itemize}
\item
\item
\end{itemize}
 
 Right-to-left implication can be then generalized to the following statement, more suited to the induction

\begin{proposition}\label{prop:main-right-to-left}

\end{proposition}

In the rest of this section, we will define first-order transductions not only for trees but for any kind of models (section~\ref{}), encode datatypes as models over a well chosen vocabulary (section~\ref{}) an finally prove Proposition~\ref{prop:main-right-to-left} (section~\ref{}).

\subsection{First-order transductions}


\subsection{Datatypes as models.}
Let us show how to encode datatypes as models. 
