...
Among our prime functions, the less satisfactory one is certainly the unfloding function, as discussed in Section~\ref{}. In this section, we will decompose it into a collection of functions containing no form of iteration. 

Here is a road map for this part of the appendix.
\begin{enumerate}
\item We first explain in Section~\ref{eq:twist} why the monotonicity condition is important for the unfolding function. This section is unrelated to the decomposition issues, and can be skept by the reader only interested in decomposing the unfolding function.
\item In Section~\ref{sec:functions-decomposing-unfolding}, we present a collection of functions which are intended to replace the unfolding function. We show that each of these functions is a first-order transduction.
\item Finally we show in Section~\ref{sec:proof-decompose-unfold} that the unfolding can be derived from the functions introduced in Section~\ref{sec:functions-decomposing-unfolding} and our prime functions presented in Section~\ref{}. 
\end{enumerate}
...
\subsection{New datatypes and prime functions to replace the unfolding}\label{sec:functions-decomposing-unfolding}
