...
\begin{definition}[Types] \label{def:types} The following ranked sets are called \emph{types}:
    \begin{enumerate}
        ...
        \item \emph{Folding.} If $\rSigma$ is a type and $k \in \set{2,3,\ldots}$ then $\reduce k \rSigma$ is a type. Elements of this set are called $k$-folds of $\rSigma$. A $k$-merge of arity $n$ consists of an element   $a \in \rSigma$ of arity $nk$ and  a function
        \begin{align*}
            f : \set{1,\ldots,nk} \to \set{1,\ldots,n} \qquad \text{every inverse image $f^{-1}(i)$ has size exactly $k$}
        \end{align*}
        We write $a/f$ for the $k$-merge given by $a$ and $f$.  We draw $k$-folds like this:
        \mypic{53}
