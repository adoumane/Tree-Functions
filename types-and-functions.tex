...
All datatypes represent ranked sets, i.e.~each element of a datatype has an arity.

\paragraph*{Atomic datatypes.} The atomic datatypes are finite ranked sets, i.e.~those with finitely many elements, and the  \emph{terminal ranked set}  defined by
\begin{align*}
\termset \eqdef  \underbrace{\redset{\naryterm 0, \naryterm 1, \naryterm 2,\ldots}.}_{\substack{\text{for each arity $i$, one} \\ \text{element denoted by $i$}}}
\end{align*}  
The reason for the $\termset$ notation is that the terminal set can be used to model partial functions: a partial function with output  type  $\rSigma$ can be seen as a total function of output type $\rSigma + \termset$, which uses  $\termset$  for  undefined values.
...
Terms form a monad, in the category of ranked sets and arity-preserving functions (see~\cite[Section 9.2]{bojanczykRecognisableLanguagesMonads2015} for a similar monad). The unit of the monad, which is an operation of type $\ranked{\rSigma \to \tmonad \rSigma}$, is illustrated in the following picture:
\mypic{98}
...
\paragraph*{Functoriality.} This completes the list of  datatype constructors.  All constructors are functors, in the sense  that  arity preserving functions
\begin{align*}
...
\begin{definition}[Datatypes and derivable function]\label{def:derivable-function}
\label{def:types} 
% \ 
%  \begin{enumerate}
    % \item 
    ...
    \end{align*}
    to ranked sets which are either finite or the terminal set $\termset$.
