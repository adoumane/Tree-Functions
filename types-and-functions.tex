...
\begin{definition}[Types] \label{def:types} The following ranked sets are called \emph{types}:
    \begin{enumerate}
        ...
        \item \emph{Arity reduction.} If $\rSigma$ is a type and $k \in \set{1,2,\ldots}$ then $\reduce k \rSigma$ is a type. An $n$-ary element of this type consists of an $a \in \rSigma$ of arity $nk$ and a function
        \begin{align*}
