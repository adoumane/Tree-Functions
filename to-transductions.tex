...
whose elements are used as the ports, and interpreting the binary relation $R$ as
        ...
        \item \emph{Terms.}  Terms in $\tmonad \rSigma$ are modelled using vocabulary of $\rSigma$ extended with two fresh binary relations $\anceord$ and $\portord$. 
          Let $t \in \tmonad \rSigma$. The model of $t$ is defined by taking the disjoint union of the models 
            \begin{align}\label{eq:non-port}
                 \coprod_{x \in \text{non-port nodes in $t$}} \underline{a(x)},
            \end{align}
         where  $\underline a(x)$ is the model over the vocabulary of $\rSigma$ that  is defined by induction assumption.   In the above  disjoint union, the same vocabulary, namely the vocabulary of $\rSigma$,  is used  for all parts of the disjoint union. 
The ports of $\underline{a}$ are defined as follows: if the $i$-{th} child of a node $x$ of $a$ is a port, then the $i$-{th} port of $\underline{a(x)}$ is a port of $\underline{a}$. The order of these ports, $\sqsubset$, is inherited from the ports of $a$.  The descndent relation $\anceord$ consists of the pairs $(u,v)$ where $u$ is the $i$-th port of $\underline{a(x)}$, $v$ is a port of $\underline{a(y)}$ and $y$ is a descendent of the $i$-th child of $x$.
...
The following lemma immediately yields the right-to-left implication in Theorem~\ref{thm:main}. Its proof, which is a straightforward case analysis of all atomic functions and combinators, is in the appendix. 
