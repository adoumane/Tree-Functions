...
Fix a first-order register transducer, with registers names $\regnames$ and output alphabet $\rGamma$. We suppose that it has $k$ registers and the set f names $\regnames$ is $[1,k]$.
...
\begin{align*}
r_i(t_1,\dots,t_n) \mapsto @(@(\dots @(\portletter, t_1)\dots,t_{n-1}),t_n)
\end{align*}
