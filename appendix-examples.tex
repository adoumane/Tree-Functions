...
\noindent\begin{example}[Partial unfold] If $\rGamma$ and $\rDelta$ are types, we define $\ranked{\Gamma*\Delta}$ to be $\ranked{\Gamma.(1+\Delta)}$. We call its inhabitants the \emph{partial shallow} terms. A partial shallow term looks like this, where we omitted to draw the element 1:
\begin{center}
...
\noindent\begin{example} Consider the function $\ranked{f}$ which makes the following diagram commute.
 ...
 \begin{tikzpicture}
  ...
  {
     \ranked{\tmonad (\reduce k \Sigma)^k} & \ranked{(\tmonad \Sigma)^{[k]}} \\
     \ranked{(\tmonad \Sigma)^{[k]})} &  \\};
  \path[-stealth]
    (m-1-1) edge node [right] {} (m-2-1)
    (m-2-1) edge node [right] {$\ranked{\mathsf{unfold}}$}  (m-1-2)
     (m-1-1) edge node [above] {$\ranked{f}$}  (m-1-2);
\end{tikzpicture}
 \end{center}
