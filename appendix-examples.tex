...
\begin{align*}
\ranked{h: \tmonad 1 \to \tmonad(\Gamma\otimes(\Gamma_0+0))}
\end{align*}
...
Now consider the function $$\ranked{\mathsf{Children}:\tmonad\rGamma\to \tmonad (\rGamma\otimes (\rGamma_0+0)^*)}$$ which tags every node of a term in $\tmonad \rSigma$ by the list  of its children symbols. When a child is a port, it is marked by $0$ in the list.
The function $\ranked{\mathsf{Sibling}}$ can be derived using a similar construction as above.
\end{example}
...
\medskip
\noindent\begin{example}[Descendant and ancestors]\label{ex:descendant} If $\rSigma$ is a finite type and $\rGamma\subseteq \rSigma$, then the functions 
\begin{itemize}
...
Note here the importance of the basic function $\decfact$. If we had only the factorization $\ancfact$, we would not be able to separate (at least easily) the red blocks having a blue descendant from the others in the figure above.  
\end{example}
