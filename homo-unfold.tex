...
%\begin{example}\label{ex:ReduceDegreeRecoverType}
%When we reduce the degree of a fold, we append a $\cdot(0+1)$, to the input type which is annoying since we lose the type. However, for certain types, we can overcome this problem. For instance, we can derive this, if $\rSigma$ contains a nullary symbol:
%\begin{align*}
%\ranked{\reduce k \tmonad \Sigma\to \reduce {k-l} \tmonad\Sigma }
%\end{align*}
%Using distributivity of $\cdot$ over $\otimes$, we can also derive this
%\begin{align*}
%\ranked{\reduce k (\tmonad \Sigma)^m\to \reduce {k-l} (\tmonad\Sigma)^m}
%\end{align*}
%\end{example}
%
%\begin{example}
%Similarly, the unfold operation appends
%\end{example}
